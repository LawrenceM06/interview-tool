...
if not st.session_state.setup_complete:
    ...
    st.session_state["experience"] = st.text_area(label="Experience", value="", height=None, max_chars = 20, placeholder="Describe your experience")
    ...
    with col2:
        st.session_state["position"] = st.selectbox(
            "Choose a position",
            {"Data Scientist", "BI Analyst", "Data Engineer"}
        )
...
if st.session_state.setup_complete and not st.session_state.feedback_shown and not st.session_state.chat_complete:
    ...
    if not st.session_state.messages:
        st.session_state.messages = [{"role":"system", "content":(f"you are an HR executive that interviews an interviewee called {st.session_state["name"]} with exprience{st.session_state["experience"]} and skills {st.session_state["skills"]}." 
